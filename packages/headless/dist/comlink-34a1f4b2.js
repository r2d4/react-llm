...
async function detectGPUDevice() {
  if (typeof navigator !== "undefined" && navigator.gpu !== void 0) {
    ...
    if (requiredMaxStorageBufferBindingSize > adapter.limits.maxStorageBufferBindingSize) {
      throw Error(`Cannot initialize runtime because of requested maxStorageBufferBindingSize exceeds limit. requested=${computeMB(requiredMaxStorageBufferBindingSize)}, limit=${computeMB(adapter.limits.maxStorageBufferBindingSize)}. `);
    }
  ...
  clear() {
    ...
    const renderBindingGroup = this.device.createBindGroup({
      ...
      entries: [{
        ...
        binding: 1,
        resource: this.stagingTexture.createView()
      }]
    });
...
class Instance {
  ...
  toPackedFuncInternal(func, autoAttachToScope) {
    if (this.isPackedFunc(func)) return func;
  ...
  *
  ...
  async fetchNDArrayCache(ndarrayCacheUrl, device) {
    ...
    await this.fetchNDArrayCacheInternal(ndarrayCacheUrl, list["records"], device);
    this.cacheMetadata = {
      ...this.cacheMetadata,
      ...list["metadata"]
    };
  ...
  showImage(dataRGBA) {
    ...
    this.lib.webGPUContext?.drawImageFromBuffer(dataRGBA.getDataPtr(), dataRGBA.shape[0], dataRGBA.shape[1]);
  }
  /**
   * Clear canvas
   */
  ...
  async asyncLoadWebGPUPiplines(mod) {
    if (this.lib.webGPUContext == void 0) throw Error("WebGPU not initialied");
...
class LLMInstance {
  ...
  async init(cb) {
    ...
    this.tvm = await instantiate(new Uint8Array(wasmSource),
    //@ts-ignore
    ...
    this.model = this.tvm.withNewScope(() => {
      return new LLMInstanceScope(this.tvm, this.tokenizer, this.config.maxWindowSize);
    });
...
`;
    ...
    tokens.unshift(...(await this.tokenizer.encodeIds(conversation.systemPrompt)));
...
export { LLMInstance as L, detectGPUDevice as d, expose as e, proxy as p, wrap as w };
